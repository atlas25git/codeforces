...
void dfs(int i,int dp,vector<bool>& vis,vector<vector<int>>& l,int& ans)
{
    vis[i] = true;
    ans = max(dp,ans);
    ...
    {
        if(!vis[x])
            dfs(x,dp+1,vis,l,ans);
    }
}

void solve(){
    int i,j,k,n,m,ans=0,cnt=0,sum=0;
        vector<vector<int>> l(2002);
        cin>>n;
        vector<int> nodes(istream_iterator<int>(cin),{});
        for(int i=1;i<=n;i++){
            if(nodes[i-1]!=-1)
                l[nodes[i-1]].push_back(i);
        }
        vector<bool>vis(2002,false);

        for(int i=0;i<n;i++)
            if(!vis[i+1] && nodes[i]==-1)
                dfs(i+1,1,vis,l,ans);
        cout<<ans<<endl;
}
void init() {
